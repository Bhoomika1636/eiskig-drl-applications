...
from typing import Any, List, Callable, Mapping, Sequence
...
class SupplysystemB(BaseEnvSim):
    ...
    def runtime_counter(self, u_old, u_new, old_runtime):
    ...
    def action_masks(self) -> List[bool]:
        """
        This methods returns a list for each action. If the entry is True, the action is valid and can be sampled from the agent.
        """
        # init runtime counter

        """
        try:
            self.my_runtime_counter
        except:
            self.my_runtime_counter = {"CHP": 0}

        # check for chp
        # valid_actions_chp = [True, True, True] #(prob 0,prob 1, prob 2) - (down, equal, up)
        try:
            CHP_runtime = self.runtime_counter(u_old=self.state_backup["u_combinedheatpower"], u_new=self.state["u_combinedheatpower"], old_runtime=self.my_runtime_counter["CHP"])
            print("u_old:",self.state_backup["u_combinedheatpower"])
            print("u_new:",self.state["u_combinedheatpower"])
            print("CHP_runtime", CHP_runtime)
        except:
            CHP_runtime = 0 # this is needed for the first step
        if CHP_runtime == 0 or CHP_runtime > 60*60*3: #3 hours
            valid_actions_chp = [True, True, True]
        else: #minimum runtime not reached
            if self.state["u_combinedheatpower"] == 0.5:
                valid_actions_chp = [False, True, True] #decrease not allowed
            else:
                valid_actions_chp = [True, True, True] #everything allowed
        
        self.my_runtime_counter.update({"CHP": CHP_runtime})

        # for now, implement a easy mask, where the boiler is only allowed to decrease the operating state, while the chp is running
        # if self.state["s_u_combinedheatpower"] > 0:
        #     valid_actions_condensingboiler = [False, False, True] 
        # else:
        #     valid_actions_condensingboiler = [True, True, True]        
        valid_actions_condensingboiler = [True, True, True]   

        #create the final List
        valid_actions_other = [True]*(self.n_action_space*3 - 3 - 3)
        valid_actions = valid_actions_chp + valid_actions_condensingboiler + valid_actions_other
        """

        # init
        try:
            self.action_mask_heat_active
        except:
            self.action_mask_heat_active = False
            self.action_mask_cold_active = False 

        # init action masks chp, condensingboiler, immersionheater, heatpump, coolingtower, compressionchiller
        valid_actions_chp = [True, True, True] # - (down, equal, up)
        valid_actions_condensingboiler = [True, True, True]
        valid_actions_immersionheater = [True, True, True]
        valid_actions_heatpump = [True, True, True]
        valid_actions_coolingtower = [True, True, True]
        valid_actions_compressionchiller = [True, True, True]

        # create masks used later
        turn_on_mask = [False, False, True] # only up is valid
        turn_off_mask = [True, False, False] # only down is valid

        # set limits
        T_heat_upper_limit = 90 # +2
        T_heat_lower_limit = 70 # -2
        T_cold_upper_limit = 17 # +2
        T_cold_lower_limit = 5 # -2
        ...
        # warm net check
        if T_heat_max > T_heat_upper_limit + 2 or (T_heat_max > T_heat_upper_limit and self.action_mask_heat_active):
            valid_actions_chp = turn_off_mask
            valid_actions_condensingboiler = turn_off_mask
            valid_actions_immersionheater = turn_off_mask
            valid_actions_heatpump = turn_off_mask
            self.action_mask_heat_active = True

        elif T_heat_min < T_heat_lower_limit -2 or (T_heat_min < T_heat_lower_limit and self.action_mask_heat_active):
            valid_actions_chp = turn_on_mask
            valid_actions_condensingboiler = turn_on_mask
            valid_actions_immersionheater = turn_on_mask
            valid_actions_heatpump = turn_on_mask
            self.action_mask_heat_active = True
        else:
            self.action_mask_heat_active = False
            
        # cold net check
        if T_cold_max > T_cold_upper_limit + 2 or (T_cold_max > T_cold_upper_limit and self.action_mask_cold_active):
            valid_actions_coolingtower = turn_on_mask
            valid_actions_compressionchiller = turn_on_mask
            valid_actions_heatpump = turn_off_mask
            self.action_mask_cold_active = True
           
        elif T_cold_min < T_cold_lower_limit - 2 or (T_cold_min < T_cold_lower_limit and self.action_mask_cold_active):
            valid_actions_coolingtower = turn_off_mask
            valid_actions_compressionchiller = turn_off_mask
            valid_actions_heatpump = turn_off_mask
            self.action_mask_cold_active = True
        else:
            self.action_mask_cold_active = False

        self.action_maks_active = self.action_mask_heat_active or self.action_mask_cold_active

        #combine total action mask
        valid_actions = valid_actions_chp + valid_actions_condensingboiler + valid_actions_immersionheater + valid_actions_heatpump + valid_actions_coolingtower + valid_actions_compressionchiller
        return valid_actions
    
    # def valid_action_mask(self) -> List[bool]:
    #     print("numer of actions:", self.n_action_space)
    #     valid_actions = [True]*self.n_action_space
    #     return valid_actions

    # def get_attr(self, attr_name):
    #     """
    #     Gets the attribute or method with the given name from the environment.

    #     :param attr_name: Name of the attribute or method to get.
    #     :return: The attribute or method with the given name.
    #     :raises AttributeError: If the attribute or method does not exist.
    #     """
    #     if hasattr(self, attr_name):
    #         return getattr(self, attr_name)
    #     else:
    #         raise AttributeError(f"Attribute or method '{attr_name}' not found in SupplysystemB.")
    ...
    def calc_reward(self):
        ...
        # give a positive reward, when action mask was active to show agent it was good to select this action
        # if self.action_maks_active:
        #     self.state["reward_other"] = self.policyshaping_costs
    ...
    def convert_disc_action(self, action_disc):
        ...
        """
        float_action = []
        # print("agent actions:", action_disc)
        # print("old idx:", self.action_disc_index)   
        ...
        # print("new idx:", self.action_disc_index)
        # print("new floats:", float_action)
