...
all_files_ag1 = glob.glob(path + 'rb2_*episode.csv') # 195900638

all_files_ag1 = glob.glob(path + 'agent_8*episode.csv') # 234072338
...
# build eval df 
# temp differences: -330778 rl vs. -614435 rb --> 283657diff,,, year: -2019349rb vs -1874724rl --> 144625diff
...
eval_df['difference_cold'] = combined_df['Temperature_cwCircuit_in'] - combined_df['Temperature_cwCircuit_out']
eval_df['difference_cool2'] = combined_df['T_in_coolingCircuit'] - combined_df['T_out_coolingCircuit']
eval_df['difference_cool1'] = combined_df['T_main_vorlauf'] - combined_df['T_main_ruecklauf']
...
print(f"Diff_cold: {eval_df['difference_cold'].sum()+eval_df['difference_cool2'].sum()}")
print(f"CER: {eval_df['Q_removed'].sum()/(1000*combined_df['P_el_ges_kw'].sum())}")
print(f"Demand: {combined_df['CoolingDemandCoolingWater'].sum() + combined_df['CoolingDemandColdWater'].sum()}")
