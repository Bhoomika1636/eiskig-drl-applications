...
class BoschSystem(BaseEnvSim):
    ...
    fmu_name = "BoschCoolingSystem_weather"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        state_var_tuple = helpers.getStateVars('experiments_hr\BoschCoolingSystem2\environment\BoschStateVars.csv')
    ...
    def step(self, action: np.ndarray) -> StepResult:
        ...
        if (self.n_steps % int(3600*1.5/self.sampling_time)) == 0:
    ...
    def render(self, mode="human", name_suffix=""):
        ...
        for i in self.episode_df.columns:
            try:
                ...
                self.episode_df.drop(i,inplace=True)
        fig = px.line(self.episode_df, x=self.episode_df.index, y=self.episode_df.columns)#["T_tank_cw","Tank_level_warm","u_P1","Temperature_cwCircuit_in",'T_in_coolingCircuit']
        fig.write_html(os.path.join(
                self.path_results,
                self.config_run.name
                + "_"
